 ...
 * @param $active_id int current active navigation element id
...
function get_navigation($template, $active_id){
    ...
    for ($x = 1; $x <= 4; $x++) {
        //als active_id gelijk is aan item
        $template[$x];
        if ($active_id == $x){
            $navigation_exp .= '<li class="nav-item active">';
            $navigation_exp .= '<a class="nav-link" href="'.$template[$x]['url'].'">'.$template[$x]['name'].'</a>';
        }else{
...
function get_serie_table($series, $pdo){
    ...
    foreach($series as $key => $value){
        ...
        <tr>
            ...
            <td scope="row">'.get_user_name($pdo, $value['user']).'</td>
...
function update_serie($pdo, $serie_info){
    ...
    if($serie_info['user'] != get_user_id()){
        return [
            'type' => 'success',
            'message' => sprintf("You're not authorized to edit this serie.")
        ];
    }
...
function remove_serie($pdo, $serie_id){
    ...
    if($serie_info['user'] != get_user_id()){
        return [
            'type' => 'success',
            'message' => sprintf("You're not authorized to remove this serie.")
        ];
    }
 ...
 * @param $pdo
 * @param $user_id
 * @return string of first and last name of user given the user id
 */
function get_user_name($pdo, $user_id){
    $stmt = $pdo->prepare('SELECT firstname, lastname FROM users WHERE id = ?');
    $stmt->execute([$user_id]);
    $names_array = $stmt->fetch();
    return $names_array["firstname"]." ".$names_array["lastname"];
}

/**
 * Get amount of active users
 * @param $pdo
 * @return string of amount of active users
 */
